...
input_file <- "Data de Driehoek.xlsx"
output_file <- "USP de Driehoek species richness.xlsx"
...
df1 <- read_excel(input_file, sheet = "Metadata")
df2 <- read_excel(input_file, sheet = "Species data March and May")

df1$Plotcode <- as.numeric(df1$Plotcode)
df2$Plotcode <- as.numeric(df2$Plotcode)
...
# --- SECOND SHEET: Unique Species per Plotcode (from Species data March+May) ---
